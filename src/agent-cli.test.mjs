...
assert.equal(serverRecoveryEvent?.recommended_action, 'review_invalid_control_traffic');
assert.equal(serverRecoveryEvent?.recommended_command, 'narada-agent-cli --identity narada.test --session server-test --session-recovery');
assert.equal(serverRecoveryEvent?.recovery_kind, 'invalid_control_review');
assert.equal(serverRecoveryEvent?.handoffs?.session_recovery, 'narada-agent-cli --identity narada.test --session server-test --session-recovery');
const serverPreflightRecoveryEvent = serverEvents.find((event) => event.event === 'preflight_recovery' && event.request_id === 'preflight-1');
assert.equal(serverPreflightRecoveryEvent?.event, 'preflight_recovery');
...
degradedChild.stdin.write(`${JSON.stringify({ id: 'recovery-degraded-1', method: 'session.recovery', params: {} })}\n`);
degradedChild.stdin.write(`${JSON.stringify({ id: 'preflight-degraded-1', method: 'preflight.recovery', params: {} })}\n`);
...
assert.equal(degradedEvents.some((event) => event.event === 'session_recovery' && event.request_id === 'recovery-degraded-1' && event.mcp_startup_failure_count === 1 && event.recommended_action === 'review_startup_diagnostics' && event.recovery_kind === 'startup_diagnostic_review' && event.recommended_command === 'narada-agent-cli --identity narada.test --session degraded-server-test --session-recovery'), true);
assert.equal(degradedEvents.some((event) => event.event === 'preflight_recovery' && event.request_id === 'preflight-degraded-1' && event.mcp_preflight_operational_state === null), true);
...
assert.deepEqual(renderOperatorEvent({ event: 'assistant_message', agent_id: 'narada.test', content: 'line one\nline two' }, { streamedTurns: new Set(), now: '2026-06-22T13:24:00.000Z' }), ['narada.test:', '  line one', '  line two 2026-06-22Z13:24']);
